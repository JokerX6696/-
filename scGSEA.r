...
# 读取 rds
sc_data <- readRDS('Epithelium_data_ob_v3.rds')
sc_data <- subset(x = sc_data,sub_celltype=='Goblet_cells')
cell_type <- 'Goblet_cells'
...
# 结果保存
res <-  data.frame(ret)
write.csv(res,paste0(cell_type,'_GSEA','.csv'))
#gseaplot(x = ret,geneSetID = 1,title = ret@result$ID[1],by = 'runningScore')
for (i in seq_along(ret@result$ID)) {
  gseaplot2(x = ret,geneSetID = i,color="red",pvalue_table = T,title="",base_size=10,ES_geom="line")
}
