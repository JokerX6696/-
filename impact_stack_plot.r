...
otu1 <-read.table(file,header=T,check.names=FALSE,sep="\t")
...
df$type <- factor(x=df$type,levels = rownames(dat))

p <- ggplot(data = df, mapping = aes(x=sample,y=value,fill=factor(type,levels = rownames(dat)),
  stratum=factor(type,levels = rownames(dat)),alluvium=factor(type,levels = rownames(dat)))) + 
